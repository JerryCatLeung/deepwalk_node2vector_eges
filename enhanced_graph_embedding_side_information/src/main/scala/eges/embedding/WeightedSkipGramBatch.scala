...
class WeightedSkipGramBatch extends Serializable {
  ...
  /**
    ...
    *
    * */
  def setBatchNum(value:Int):this.type = {
    require(value > 0, s"batchNum must be more than 0, but it is $value")
    batchNum = value
    this
  }
  ...
  def fit(dataSet: RDD[Array[Array[String]]]): RDD[(String, String)] = {
    ...
    // 数据分组
    val batchWeightArray = Array.fill(batchNum)(1.0/batchNum)
    val dataSetBatch = dataSetRepartition.randomSplit(batchWeightArray)
    val perBatchCountArray = Array.fill[Int](batchNum)(0)
    for(batchA <- 0 until batchNum){
      val batchCount = dataSetBatch(batchA).count().toInt
      perBatchCountArray(batchA) = batchCount
      if(batchCount > 0) dataSetBatch(batchA).persist(StorageLevel.MEMORY_AND_DISK)
    }
    ...
    for(iter <- 0 until iterationNum){
      ...
      val perBatchLossArray = ArrayBuffer[Double]()
      for(batch <- 0 until batchNum){
        if(perBatchCountArray(batch) > 0) {
          // 训练结果
          val trainResult = dataSetBatch(batch).map(k => ((new Random).nextInt(numPartitions), k))
            .repartition(numPartitions).map(k => k._2).mapPartitions(dataIterator =>
          ...
          if(isNoShowLoss){
            ...
            perBatchLossArray.append(trainLossResult)
            println(s"====第${iter+1}轮====第${batch+1}批====平均损失:$trainLossResult====")
          }
          val trainNodeSideResultUnion = trainWeightResult.union(trainVectorResult)
            .map{ case (key, (gradientVectorSum, num)) =>
              ...
              val embeddingUpdate = embeddingRaw - 10 * lambdaBroadcast.value * gradientVectorSum / num.toDouble
      ...
      // 计算每轮平均损失
      val batchLossLength = perBatchLossArray.length
      if(isNoShowLoss && batchLossLength > 0){
        println(s"====第${iter+1}轮====平均损失:${perBatchLossArray.sum/batchLossLength}====")
      }
    }
    for(batchB <- 0 until batchNum){
      if(perBatchCountArray(batchB) > 0) dataSetBatch(batchB).unpersist()
    }
